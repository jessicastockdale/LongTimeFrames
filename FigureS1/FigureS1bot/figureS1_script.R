...
library(nonpar)
library(ggpubr)
source("sd_funcs_S1.R")
set.seed(12234911) #LTE
...
dd[[1]] = matrix(NA, nrow=length(ss)-1, ncol = niter)
  ...
  # dd[[4]] = matrix(NA, nrow=length(ss)-1, ncol = niter)
...
# if (run==1){
  ...
  #qq[[4]] = matrix(NA, nrow= nrow(dd[[1]]), ncol=3)
  # 
